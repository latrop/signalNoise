...
class ImgCat:
    ...
    def makecat(self, rerun=True, keepcat=False, verbose=True, polarMode=None):
        self.cat = pysex.run(self.filepath, conf_args={'DETECT_THRESH': 3.0, 'ANALYSIS_THRESH': 3.0,
                             ...
                             rerun=rerun, keepcat=keepcat, catdir="alipy_cats", polarMode=polarMode)
    ...
    def showquads(self, show=False, flux=True, verbose=True):
        ...
        if len(self.quadlist) != 0:
            for qd in self.quadlist:
                polycorners = star.listtoarray(qd.stars)
