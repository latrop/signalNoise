...
class Identification:
    ...
    def calcfluxratio(self, verbose=True):
        ...
        if verbose:
            print("Computed flux ratio from %i matches : median %.2f, std %.2f" % (len(reffluxes), self.medfluxratio,
                                                                                   self.stdfluxratio))
...
def run(ref, ukns, hdu=0, visu=True, skipsaturated=False, r=5.0, n=500, sexkeepcat=False, sexrerun=True,
        verbose=True, polarMode=None, refpolar=False):
    """
    ...
    for ukn in ukns:
        ...
        ukn.makecat(rerun=sexrerun, keepcat=sexkeepcat, verbose=verbose, polarMode=polarMode)
