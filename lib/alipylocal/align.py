...
def fromfits(infilename, hdu=0, verbose=True):
    ...
    if verbose:
        print("FITS import (%i, %i) BITPIX %s / %s" % (pixelarrayshape[0], pixelarrayshape[1],
                                                       hdr["BITPIX"], str(pixelarray.dtype.name)))
...
def irafalign(filepath, uknstarlist, refstarlist, shape, alifilepath=None,
    ...
    geomapblabla = iraf.geomap(input=geomapinpath, database=geodatabasepath, xmin=1,
                               xmax=shape[0], ymin=1, ymax=shape[1], Stdout=1)
    ...
    if verbose:
        print("IRAF geomap : Rotation %+11.6f [deg], scale %8.6f, RMS %.3f [pixel]" % (geomapangle, geomapscale,
                                                                                       geomaprms))
